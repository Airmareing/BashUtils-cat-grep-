...
void parser(int argc, char *argv[], opt *options) {
  ...
  if (optind == 1) {  // если нет введённых аргументов
    print_usage();
  } else if (unknown != 1) {  // если не найдено неизвестных опций
    if (!options->e) {
      if (!options->f) {
    ...
    if (options->file_num > 1) {  // если больше 1 файла - ставим спец флаг
      options->multiple_files = 1;
    }
    FILE *file;
    while ((optind < argc) &&
...
void file_flags_output(char *filename, int match_num, int inv_match_num,
  ...
  if ((options->l) || (options->c)) {
    if ((options->l) && (options->c) && (options->multiple_files)) {
      if (options->v) {
        printf("%s:%d\n", filename, inv_match_num);
      } else if (!options->v) {
        printf("%s:%d\n", filename, match_num);
      }
    } else if ((options->l) && (options->c) && (!options->multiple_files)) {
      printf("1\n%s\n", filename);
    } else {
...
void print_output(char *filename, char *line, int line_num, opt *options) {
  if ((options->multiple_files) &&
      (!options->h)) {        // если несколько файлов и !h
